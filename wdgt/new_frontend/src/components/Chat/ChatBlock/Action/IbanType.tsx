...
const ibanValidator = validators.composeValidators(
  validators.createRequiredValidator('Field is required'),
  validators.createIbanValidator('IBAN number is not valid'),
);

const getInitialInputValue = (action: IAction, selected?: ISelectedAction): FormValues => {
  const pureValues = { iban_number: '' };
...
const IbanType: React.FC<Props> = (props) => {
  const {
    action,
    selected,
    actionDisabled,
  ...
  function mapStateToRedux(values: FormValues): IStateFreeText {
    return {
      message: values.iban_number.trim(),
    }
  }

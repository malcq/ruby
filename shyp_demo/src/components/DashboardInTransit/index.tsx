...
import {fetchDashboardDataTransit} from "../../stores/actionCreators/dashboard";
import { CircularProgress } from '@material-ui/core';

const mapStateToProps = (state: IGlobalState): any => ({
  count: state.dashboard.in_transit_count,
  shipments: state.dashboard.in_transit.shipments,
  loading: state.dashboard.in_transit.loading,
});

const mapDispatchToProps = ( dispatch: Dispatch ) => ({
  getData: promisifyAction(dispatch, fetchDashboardDataTransit)
});
...
class DashboardDelivered extends PureComponent<any, any> {
  ...
  public render () {
    return (
      ...
      >
        <div className="dashboard-layout__title icon dashboard">
        ...
        { !this.props.loading && !this.state.full && this.props.count > 5 && (
          <div className="dashboard-layout__load-more" onClick={this.loadMore}>
            Show all {this.props.count} in transit shipments
          </div>
        )}
        { !this.props.loading && this.state.full && this.props.count > 5 && this.state.expanded && (
          <div className="dashboard-layout__load-more" onClick={this.expand}>
            Show first 5 transit shipments
          </div>
        )}
        { !this.props.loading && this.state.full && this.props.count > 5 && !this.state.expanded && (
          <div className="dashboard-layout__load-more" onClick={this.expand}>
